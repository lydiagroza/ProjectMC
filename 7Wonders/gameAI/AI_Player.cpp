...
std::string GameState::toHash() const {
    ...
    int oppMilitaryThreat = (std::abs(oppMilitaryPosition) >= 6) ? 1 : 0;
...
void QLearningAgent::loadModel(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (actionStr.find("BUY_") == 0) {
            ...
            action.cardId = std::stoi(actionStr.substr(4));
            action.wonderId = 0;
        } else if (actionStr.find("WONDER_") == 0) {
            action.type = Action::BUILD_WONDER;
            size_t underscorePos = actionStr.find("_CARD_");
            action.wonderId = std::stoi(actionStr.substr(7, underscorePos - 7));
            action.cardId = std::stoi(actionStr.substr(underscorePos + 6));
        } else if (actionStr.find("DISCARD_") == 0) {
            action.type = Action::DISCARD_CARD;
            action.cardId = std::stoi(actionStr.substr(8));
            action.wonderId = 0;
        } else {
...
std::vector<std::shared_ptr<CardBase>> AI_Player::getPlayableCards(
    const Board& board, const Player& opponent) const {
